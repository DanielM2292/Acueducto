...
@matriculas_bp.route('/asociar_matricula', methods=["PUT", "OPTIONS"])
def asociar_matricula():
    if request.method == 'OPTIONS':
        return jsonify({'message': 'CORS preflight response'}), 200
    data = request.get_json()
    return MatriculasServices.asociar_matricula(data)
...
@matriculas_bp.route('/buscar_matricula_por_documento', methods=["GET", "OPTIONS"])
